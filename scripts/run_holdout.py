"""T3.27 phase gate: score the hand-labelled holdout with the current
pipeline, unchanged, and report all five metrics side by side with the
T2.21 synthetic baseline. Not part of the shipped package -- a one-off
runner for this specific gate, mirroring run_baseline.py."""
...
ir_dir = Path("corpus/holdout/ir")
img_dir = Path("corpus/holdout/img")
...
for ir_path in sorted(ir_dir.glob("*.json")):
    ...
    except Exception as exc:  # noqa: BLE001 -- gate runner, not shipped code
...
print(format_scorecard(aggregated, title=f"Holdout scorecard ({len(results)} diagrams)"))
...
if failures:
    print()
    print(format_failure_breakdown(failures, title="Holdout failure breakdown"))

# T2.21 synthetic baseline, for the side-by-side comparison T3.27 requires.
baseline = {
    "class_recall": 1.000,
    "member_f1": 0.778,
    "relationship_f1": 0.873,
    "pair_recall": 0.984,
    "kind_accuracy_given_correct_pair": 0.880,
}
print("\n" + "=" * 70)
print("Corpus (T2.21) vs holdout (T3.27)")
print("-" * 70)
for metric_name, corpus_value in baseline.items():
    holdout_value = getattr(aggregated, metric_name)
    gap = holdout_value - corpus_value
    print(f"{metric_name:38s} corpus={corpus_value:6.1%}  holdout={holdout_value:6.1%}  gap={gap:+6.1%}")
